...
Each implementation task will be automatically graded by our test suite on GradeScope. You may resubmit your code an unlimited number of times for feedback. Each task is allocated a set number of points based on its complexity. To receive full points for a task, you must pass all tests. Some tasks have many tests, and you may receive partial marks on a task as noted in the list below:

* Task 1.1: Add an HTTP route to create conversation areas (X points)
  * Full marks: Pass all tests
  * 50%: Pass at least M tests
* Task 1.2: Implement business logic to create a conversation area (Y points)

**TODO** add a full rubric once the tests are written.

**TODO** add some rubric for the manual grading

Your code will be manually evaluated for conformance to our course [style guide]({{ site.baseurl }}{% link style.md %}). This manual evaluation will account for 10% of your total grade on this assignment.
...
## Part 1: Base implementation of TownService's ConversationArea API
...
### Task 1.1: Add an HTTP route to create conversation areas
...
### Task 1.2: Implement business logic to create a conversation area
With the top of the architecture stack implemented, it's now time to implement some business logic to actually create a conversation area when the `conversationAreaCreateHandler` is called. This handler should:
...
### Task 1.3: Include current conversations in join response 
