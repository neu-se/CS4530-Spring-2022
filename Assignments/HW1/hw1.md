...
[Download Avery's code for the backend module here]({{ site.baseurl }}).
...
```
We will cover these keywords and their implications in detail [in week 4]({{ site.baseurl }}/schedule.md). 
...
* Conform to our [style guide]({{ site.baseurl }}/WAS:style.md) *and* have no style warnings or errors as reported by `npm run-script lint`
